...
function formatPrice(amount) {
  ...
  let numericAmount = Math.round(Number(amount) || 0);

  // SAFETY: Prevent overflow - max reasonable price is 999 billion
  const MAX_SAFE_PRICE = 999999999999;
  if (numericAmount > MAX_SAFE_PRICE || !Number.isSafeInteger(numericAmount)) {
    console.warn('[formatPrice] Price overflow detected:', amount);
    numericAmount = MAX_SAFE_PRICE;
  }
...
/**
 * Bind carousel events (scroll left/right)
 * @param {Object} options - Carousel options
 * @param {HTMLElement} options.track - Scrollable track element
 * @param {HTMLElement} options.prev - Previous button element
 * @param {HTMLElement} options.next - Next button element
 * @param {string} options.itemSelector - Selector for items (to calculate width)
 */
function bindCarousel(options) {
  const { track, prev, next, itemSelector } = options;
  if (!track || !prev || !next) return;

  const scrollAmount = () => {
    const item = track.querySelector(itemSelector);
    return item ? item.offsetWidth + 20 : 300; // Default 300px if no item
  };
  ...
  track.addEventListener('scroll', () => {
    ...
    prev.style.opacity = track.scrollLeft > 0 ? '1' : '0.5';
    next.style.opacity = track.scrollLeft < maxScroll ? '1' : '0.5';
  });
}
