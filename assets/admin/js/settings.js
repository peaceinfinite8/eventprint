/* settings.js — EventPrint Admin Settings (no Cropper preview) */
...
function initSweetAlertHelpers() {
  ...
  window.toast = async function toast(icon, title) {
    const ok = await window.ensureSwal();
    if (!ok) return;
    Swal.fire({
      toast: true,
      position: 'top',
...
function initContacts() {
  ...
  function normalizeWaNumber(raw) {
    ...
    // Convert 08xxxx -> 62xxxx
    if (s.startsWith('08')) s = '62' + s.slice(1);

    // Convert 8xxxx -> 62xxxx
    if (s.startsWith('8')) s = '62' + s;

    // If starts with 0 (other formats), try convert to 62
    if (s.startsWith('0')) s = '62' + s.slice(1);

    // Basic sanity to avoid empty/too short
    if (s.length < 8) return '';

    return s;
  }

  function disableWaBtn(btn) {
    if (!btn) return;
  ...
  function enableWaBtn(btn, href) {
    ...
    // Remove role to allow native link behavior
  ...
  // Init: force update existing rows (after DOM ready)
  const initAll = () => {
    container.querySelectorAll('.contact-row').forEach((row) => updateRowState(row));
  };
  initAll();
  ...
  // Update button state on typing + on blur (blur helps when autofill/paste)
  ...
  container.addEventListener('click', async (e) => {
    ...
    if (waBtn && waBtn.classList.contains('is-disabled')) {
      e.preventDefault();
    }
  });

  // Safety: if something injects rows later, you can call this manually
  window.__refreshContactsLinks = initAll;
}
...
function initLogoCrop(initialSrc) {
  ...
  /* submit button lock & validation */
  const form = document.getElementById('settingsForm');
  const saveBtn = document.getElementById('saveBtn');
  if (form && saveBtn) {
    form.addEventListener('submit', (e) => {
      // Validate Tagline (Max 25 Words)
      const taglineInput = form.querySelector('input[name="site_tagline"]');
      if (taglineInput) {
        const words = taglineInput.value.trim().split(/\s+/).filter(w => w.length > 0);
        if (words.length > 25) {
          e.preventDefault();
          window.toast?.('error', `Gagal Simpan: Tagline terlalu panjang (${words.length}/25 kata)!`);
          taglineInput.focus();
          taglineInput.classList.add('is-invalid');
          setTimeout(() => taglineInput.classList.remove('is-invalid'), 3000);
          return;
        }
      }
...
function initGmapsPreviewRight() {
  ...
  function isAllowedEmbedUrl(url) {
    try {
      ...
      // Allow official embed endpoint
      if (u.pathname.startsWith('/maps/embed')) return true;

      // Allow output=embed fallback
  ...
  function buildFallbackEmbedSrc() {
    const addr = (addressInput ? addressInput.value : '').trim();
    if (!addr) return '';

    // Use address as query
  ...
  function updatePreview() {
    ...
    // Fallback: if embed empty, generate embed from address
    if (!src) {
    ...
    if (!isAllowedEmbedUrl(src)) {
      setPreviewSrc('');
      setError('Embed tidak valid. Gunakan embed Google Maps (/maps/embed) atau pastikan alamat terisi untuk auto-preview.');
      updateOpenLink();
      return;
    }
  ...
  if (embedInput) embedInput.addEventListener('input', onInput);
  if (linkInput) linkInput.addEventListener('input', onInput);
  if (addressInput) addressInput.addEventListener('input', onInput);
}
