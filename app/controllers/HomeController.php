...
class HomeController extends Controller
{
    ...
    public function heroIndex(): void
    {
        ...
        $items = [];
        if ($stmt = $db->prepare("SELECT * FROM hero_slides WHERE page_slug IN ('home', 'home_right_top', 'home_right_bottom') ORDER BY page_slug ASC, position ASC, id ASC")) {
            $stmt->execute();
            $rs = $stmt->get_result();
    ...
    public function heroStore(): void
    {
        ...
        $page = trim($_POST['page_slug'] ?? 'home');
    ...
    public function heroUpdate(int $id): void
    {
        ...
        $stmt = $db->prepare("UPDATE hero_slides
                          SET page_slug=?, title=?, subtitle=?, badge=?, cta_text=?, cta_link=?, image=?, position=?, is_active=?, updated_at=CURRENT_TIMESTAMP
                          WHERE id=?");
        $stmt->bind_param('ssssssssii', $page, $title, $subtitle, $badge, $ctaText, $ctaLink, $image, $pos, $active, $id);
        $stmt->execute();
        $stmt->close();
