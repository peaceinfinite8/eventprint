...
function renderCategories(categories) {
  ...
  // Initialize Desktop Carousel Logic
  setupServicesCarousel();
}

/**
 * Setup Services Carousel (Desktop Navigation)
 */
function setupServicesCarousel() {
  ...
  // Calculate scroll step (approx 3 cards)
  ...
  // Optional: Auto-disable buttons at ends
  const updateButtons = () => {
    prevBtn.disabled = track.scrollLeft <= 5;

    // Disable next if at end (approximate tolerance)
    const maxScroll = track.scrollWidth - track.clientWidth;
    nextBtn.disabled = track.scrollLeft >= maxScroll - 5;
  };

  track.addEventListener('scroll', updateButtons);
  // Initial check (delay slightly to allow rendering layout)
