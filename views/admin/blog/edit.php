...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Cari elemen img preview yang sudah ada, atau container placeholder
    const previewContainer = thumbInput.previousElementSibling.previousElementSibling || thumbInput.previousElementSibling;
    // Logic selektor di atas agak tricky karena struktur PHP if/else.
    // Mari kita buat lebih robust dengan mencari parent
    ...
    thumbInput.addEventListener('change', function (e) {
      ...
      reader.onload = function (e) {
        // Cari apakah sudah ada elemen img sebelumnya
        ...
        if (img) {
          ...
          // Replace placeholder div dengan img baru
          const newPreview = `
                  <div class="mb-2 position-relative">
                    <img src="${e.target.result}" 
                      alt="Preview" class="img-fluid rounded border shadow-sm w-100 object-fit-cover" style="max-height: 180px;">
                    <div class="position-absolute bottom-0 start-0 w-100 bg-success bg-opacity-75 text-white text-center py-1 rounded-bottom small">
                      Preview Baru
                    </div>
                  </div>
                `;
          // Ganti konten container (sebelum input) dengan preview baru
          // Kita perlu hati-hati menargetkan div yang benar.
          // Struktur: div(mb-3) > [div(preview/placeholder)] > input

          // Hapus sibling sebelum input (yaitu div preview lama atau placeholder)
          const parent = thumbInput.parentElement;
          // remove div di atas input (placeholder/img lama)
          // asumsi struktur: label, div-preview, input
          if (parent.children.length >= 3) {
            // elemen ke-1 (index 1) adalah preview/placeholder
            parent.children[1].outerHTML = newPreview;
          }
        }
      }
