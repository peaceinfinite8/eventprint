  ...
  <div class="card-body">
    <form class="row g-2 align-items-end" method="get" action="<?php echo $baseUrl; ?>/admin/discounts">
      <div class="col-md-6">
        ...
        <input class="form-control" name="q" value="<?php echo htmlspecialchars($filter_q); ?>" placeholder="nama produk / tipe">
      </div>
  ...
  <div class="card-body table-responsive">
    <table class="table table-striped align-middle">
      ...
      <?php if (!empty($items)): ?>
          ...
          <tr>
            ...
            <td>
              <?php if (($it['discount_type'] ?? '') === 'fixed'): ?>
                <span class="badge bg-dark">Rp <?php echo number_format((float)$it['discount_value'], 0, ',', '.'); ?></span>
              <?php else: ?>
                <span class="badge bg-primary"><?php echo rtrim(rtrim((string)$it['discount_value'], '0'), '.'); ?>%</span>
              <?php endif; ?>
            </td>
            <td>
              <div class="small"><?php echo (int)($it['qty_used'] ?? 0); ?> / <?php echo (int)($it['qty_total'] ?? 0); ?></div>
            ...
            <?php if ($isSuper): ?>
              <td class="text-end">
                <a class="btn btn-sm btn-primary" href="<?php echo $baseUrl; ?>/admin/discounts/edit/<?php echo (int)$it['id']; ?>">Edit</a>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <tr><td colspan="<?php echo $isSuper ? 7 : 6; ?>" class="text-muted">Belum ada diskon.</td></tr>
      <?php endif; ?>
      </tbody>
    </table>
        ...
        <ul class="pagination pagination-sm mb-0 justify-content-end">
          <?php for ($p=1; $p <= $lastPage; $p++): ?>
            <li class="page-item <?php echo $p === $page ? 'active' : ''; ?>">
              <a class="page-link" href="<?php echo $baseUrl; ?>/admin/discounts?page=<?php echo $p; ?>&q=<?php echo urlencode($filter_q); ?>">
                <?php echo $p; ?>
              </a>
            </li>
          <?php endfor; ?>
