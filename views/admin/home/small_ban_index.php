...
<div class="d-flex align-items-center justify-content-between mb-4 fade-in">
    <div>
        <h1 class="h4 mb-1 fw-bold text-gradient">Small Banners (Promo)</h1>
        <p class="text-muted small mb-0">Kelola banner promo kecil di halaman depan</p>
    </div>
    <a class="btn btn-primary shadow-sm" href="<?= $baseUrl ?>/admin/home/small-banner/create">
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-5">
                            <div class="text-muted">
                                ...
                                <p class="mb-0">Belum ada small banner.</p>
                            </div>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($items as $it): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($imgUrl): ?>
                                    <img src="<?= $imgUrl ?>" alt="Thumb" class="rounded border"
                            ...
                            <td class="text-end">
                                <div class="btn-group">
                                    <a class="btn btn-icon btn-sm text-primary"
                                        href="<?= $baseUrl ?>/admin/home/small-banner/edit/<?= $id ?>" title="Edit">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>

                                    <form method="post" action="<?= $baseUrl ?>/admin/home/small-banner/delete/<?= $id ?>"
                                        class="d-inline" onsubmit="return confirm('Hapus banner ini?')">
                                        <input type="hidden" name="csrf_token"
                                            value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>">
                                        <button class="btn btn-icon btn-sm text-danger" type="button"
                                            onclick="this.form.submit()" title="Hapus">
